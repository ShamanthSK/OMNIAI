...
class GrokProvider(AIProvider):
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.capabilities = ["general", "realtime", "reasoning", "coding"]
        self.api_key = api_key or settings.XAI_API_KEY
        self.is_configured = bool(self.api_key and len(self.api_key.strip()) > 5)
        self.demo_fallback = DemoProvider("grok", "xAI Grok", "grok-2-latest")
        self._client = None
    ...
    async def generate(self, prompt: str, system_prompt: Optional[str] = None, context: Optional[str] = None) -> str:
        ...
        try:
            messages = []
            sys_content = system_prompt or "You are Grok, an intelligent AI developed by xAI."
            if context:
            ...
            return f"{demo_res}\n\n*(Notice: Live xAI Grok API encountered error: {str(e)})*"
